...
export default function AuthPage({ mode }) {
  ...
  return <div className="auth-layout grid min-h-screen grid-cols-[1.05fr_.95fr] bg-bg text-text max-lg:grid-cols-1">
    ...
    <section className="auth-story relative flex min-h-screen flex-col justify-between overflow-hidden p-10 max-lg:hidden">
      <Link className="brand flex items-center gap-3 auth-brand" to="/"><img className="brand-mark h-10 w-10 rounded-xl object-contain" src={payLogo} alt="Pay-Panda" /><strong>Pay-Panda</strong></Link>
      <div ref={storyRef}>
    ...
    <main className="auth-panel m-auto w-[min(460px,calc(100%-32px))] rounded-3xl border border-line bg-panel p-8 shadow-elevated" ref={panelRef}>{otpState?<form onSubmit={verifyOtp}><p className="eyebrow mb-1 text-[var(--font-micro)] font-extrabold uppercase tracking-[var(--tracking-wide)] text-[var(--muted-2)] accent text-accent-contrast">Two-step verification</p><h2>Enter your login code</h2><p>We sent a six-digit OTP to <strong>{otpState.maskedEmail}</strong>. The code expires in 10 minutes.</p><label>Login OTP<OtpInput value={otp} onChange={setOtp} disabled={busy} autoFocus/></label>{error&&<div className="alert mt-4 rounded-xl px-4 py-3 text-small error border border-red/25 bg-red/10 text-red">{error}</div>}<button className="primary-button inline-flex min-h-11 items-center justify-center gap-2 rounded-[var(--radius-md)] border-0 bg-gradient-to-br from-violet-600 to-indigo-500 px-4 font-bold text-white shadow-[var(--shadow-glow-accent)] transition disabled:cursor-not-allowed disabled:opacity-60" disabled={busy||otp.length!==6}>{busy?'Verifying…':'Verify and sign in'}<ArrowRight/></button><button className="resend-button" type="button" onClick={resendOtp} disabled={busy||resendCooldown>0}>{resendCooldown>0?`Resend code in ${resendCooldown}s`:'Send a new code'}</button><button className="otp-back" type="button" onClick={()=>{setOtpState(null);setOtp('');setError('')}}>Use different credentials</button></form>:<form onSubmit={submit}>
      <p className="eyebrow mb-1 text-[var(--font-micro)] font-extrabold uppercase tracking-[var(--tracking-wide)] text-[var(--muted-2)] accent text-accent-contrast">{signup ? 'Create workspace' : 'Welcome back'}</p>
      ...
      {signup && <div className="form-grid grid grid-cols-2 gap-4 max-md:grid-cols-1"><label>Full name<input required value={form.name} onChange={e => setForm({...form,name:e.target.value})}/></label><label>Business name<input required value={form.businessName} onChange={e => setForm({...form,businessName:e.target.value})}/></label></div>}
      ...
      {signup && <div className="password-strength mt-2"><div className="strength-track h-1.5 overflow-hidden rounded-full bg-line"><i style={{width:`${strength.score*25}%`}} className={`strength-${strength.score}`}/></div><div><span>{strength.label}</span><small>Minimum 6 characters; mixed characters improve strength</small></div></div>}
